...
import time
class process_vcf:
    def __init__(self,vcf_file,lovd_file,source):
    ...
    def query_API_lovd(self,genome_build,variant_description, transcript_model="all", select_transcripts="all", checkonly="False", liftover="False"):
    ...
    def variant_validator_lovd_api_call(self):
        ...
        for selected_variant_description in variant_description:
            lovd_json=self.query_API_lovd(self.source,selected_variant_description,transcript_model="all", select_transcripts="all", checkonly="False", liftover="False")
            format_json=self.format_data(selected_variant_description,lovd_json)
            if not format_json:
                missing_variant.append(format_json)
            else:
    ...
    def format_data(self,seq,vcf_json):
        ...
        if not vcf_json or not vcf_json.get(seq,{}) or not vcf_json.get(seq,{}).get(seq,{}) or not vcf_json.get(seq,{}).get(seq,{}).get('hgvs_t_and_p', {}):
            ...
            for transript in vcf_json.get(seq,{}).get(seq,{}).get('hgvs_t_and_p', {}).values():
                if not transript and transript.get('select_status',{}):
                    print(f'{seq} has no related variant')
                else:
                    if 'refseq_select' in transript.get('select_status',{}):#here we use the key of select_statues content to judge whether the reference is from refseq or ensembl
                        refseq_variant.append(transript.get('t_hgvs',{}))
                    if 'ensembl_select' in transript.get('select_status',{}):
...
if __name__=="__main__":
    filepath_GRCh37='output\VCFS\GRCh37_indels.vcf'
    filepath_GRCh38='output\VCFS\GRCh38_indels.vcf'
    lovd_json_path_GRCh37='LOVD_json\GRCh37lovd.json'
    lovd_json_path_GRCh38='LOVD_json\GRCh38lovd.json'
    process_result_37=process_vcf(filepath_GRCh37,lovd_json_path_GRCh37,'GRCh37')
    process_result_38=process_vcf(filepath_GRCh38,lovd_json_path_GRCh38,'GRCh38')
