...
def get_highest_transcript_versions(df, output_dir, sam_name): 
    ...
    #print("group", df.groupby('transcript_base')['transcript_version'].head())
    idx = df.groupby('transcript_base')['transcript_version'].idxmax()
   # print("idx", df.groupby('transcript_base')['transcript_version'].idxmax().head()) 
    highest_versions_df = df.loc[idx] 
   # print("highest", highest_versions_df.head())
